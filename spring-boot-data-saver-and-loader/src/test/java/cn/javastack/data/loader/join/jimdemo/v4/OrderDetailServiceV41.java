 ...
 */
@Service
public class OrderDetailServiceV41 implements OrderDetailService {
    @Autowired
    ...
    public List<? extends OrderDetailVO> getByUserId(Long userId) {
        ...
        List<OrderDetailVOV41> orderDetailVOS = orders.stream()
                .map(order -> new OrderDetailVOV41(OrderVO.apply(order)))
                .collect(toList());

        this.dataLoaderInMemoryService.loaderInMemory(OrderDetailVOV41.class, orderDetailVOS);
