 ...
 */
@Service
public class OrderDetailServiceV42 implements OrderDetailService {
    @Autowired
    ...
    public List<? extends OrderDetailVO> getByUserId(Long userId) {
        ...
        List<OrderDetailVOV42> orderDetailVOS = orders.stream()
                .map(order -> new OrderDetailVOV42(OrderVO.apply(order)))
                .collect(toList());

        this.dataLoaderInMemoryService.loaderInMemory(OrderDetailVOV42.class, orderDetailVOS);
