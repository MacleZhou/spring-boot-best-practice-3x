package cn.javastack.aimdemo.infra.repository.v1;

import cn.javastack.aimdemo.domain.model.entity.Product;
import cn.javastack.aimdemo.domain.service.ProductService;
import cn.javastack.aimdemo.infra.repository.dao.entity.*;
import cn.javastack.aimdemo.infra.repository.dao.mapper.*;
import cn.javastack.aimdemo.infra.repository.dvo.*;
import cn.javastack.aimdemo.infra.repository.v1.dvo.AggregatedDVO;
import cn.javastack.aimdemo.infra.repository.dao.entity.*;
import cn.javastack.aimdemo.infra.repository.dao.mapper.*;
import cn.javastack.aimdemo.infra.repository.dvo.*;
import cn.javastack.aimdemo.infra.repository.v1.dvo.*;
...
public class ProductServiceImplV1 implements ProductService {
    ...
    public Product loadProduct(String companyCode, String productCode){
        ...
        aggregatedDVO.setProduct(productDVO);
        ...
        aggregatedDVO.setProductLimits(medicalProductLimits.stream().map(medicalProductLimit -> {
        ...
        aggregatedDVO.setProductDeducts(
        ...
        aggregatedDVO.setProductCoshares(medicalProductCoshares.stream().map(medicalProductCoshare -> {return ProductCoshareDVO.apply(medicalProductCoshare);
        }).collect(toList()));

        //5. 加载产品责任
        List<MedicalProductBenefit> medicalProductBenefits = this.medicalProductBenefitMapper.get(productCode);
        aggregatedDVO.setProductBenefits(medicalProductBenefits.stream().map(medicalProductBenefit -> {return ProductBenefitDVO.apply(medicalProductBenefit);}).collect(toList()));
        ...
        aggregatedDVO.setBenefitLimits(benefitLimits);
        ...
        aggregatedDVO.setBenefits(benefitDVOMap);
        ...
        aggregatedDVO.setLimitCategories(limitCategoryDVOMap);
